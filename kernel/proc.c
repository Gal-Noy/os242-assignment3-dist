...
uint64 map_shared_pages(struct proc *src_proc, struct proc *dst_proc, uint64 src_va, uint64 size)
{
  ...
  for (page = PGROUNDDOWN(src_va); page < PGROUNDUP(src_va + size); page += PGSIZE)
  {
    ...
    if (src_pte == 0 || !(PTE_FLAGS(*src_pte) & PTE_V) || !(PTE_FLAGS(*src_pte) & PTE_U))
    {                                                                                  // The source page is not mapped, or not valid, or not user-accessible
      unmap_shared_pages(dst_proc, PGROUNDUP(dst_proc->sz), allocated_pages * PGSIZE); // In case of an error, unmap the shared pages
      return 0;
    }

    if (mappages(dst_proc->pagetable, dst_va, PGSIZE, PTE2PA(*src_pte), PTE_FLAGS(*src_pte) | PTE_S) < 0)
    {                                                                                  // Map the source page to the destination process
      unmap_shared_pages(dst_proc, PGROUNDUP(dst_proc->sz), allocated_pages * PGSIZE); // In case of an error, unmap the shared pages
      return 0;
    }
    ...
    dst_proc->sz += PGSIZE; // Update the destination process's size
  }
